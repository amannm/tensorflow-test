...
export async function loadFaceMesh(image) {
...
export function drawFaceMask(canvas, image, predictions) {
  ...
  if (predictions.length > 0) {
    predictions.forEach(prediction => {
      if (prediction.faceInViewConfidence >= 0.01) {
        ...
        const center = [imageWidth/2, imageHeight/2, 0];

        const output = buildAsset(prediction.mesh, TRIANGULATION);
        console.log(output);
        downloadURI(JSON.stringify(output), "asset.gltf", "model/gltf+json")
...
function downloadURI(content, fileName, contentType) {
...
function buildAsset(keypoints, indices) {
  if (keypoints.length === 0) {
    throw 'cannot build asset for empty mesh';
  }
  const vertexBufferLength = keypoints.length * 3 * 4;
  const indexBufferLength = indices.length * 2;
  const padding = indexBufferLength % 4;
  const vertexBufferByteOffset = indexBufferLength + padding;
  const buffer = new ArrayBuffer(indexBufferLength + padding + vertexBufferLength);
  const indexBuffer = new Uint16Array(buffer, 0, indices.length);
  const vertexBuffer = new Float32Array(buffer, vertexBufferByteOffset, keypoints.length * 3);
  indexBuffer.set(indices, 0);
  const maxIndexValue = Math.max(...indices);
  const minIndexValue = Math.min(...indices);
  const max = [
    keypoints[0][0],
    -keypoints[0][1],
    keypoints[0][2]
  ...
  for (let i = 0;  i < keypoints.length; i++) {
    ...
    const keypointX = keypoint[0];
    const keypointY = -keypoint[1];
    const keypointZ = keypoint[2];
    max[0] = Math.max(max[0], keypointX);
    max[1] = Math.max(max[1], keypointY);
    max[2] = Math.max(max[2], keypointZ);
    min[0] = Math.min(min[0], keypointX);
    min[1] = Math.min(min[1], keypointY);
    min[2] = Math.min(min[2], keypointZ); 
    vertexBuffer.set([keypointX, keypointY, keypointZ], i*3);
  }
    ...
    "buffers" : [
      {
        ...
        "byteLength" : indexBufferLength + padding + vertexBufferLength
      }
    ],
    "bufferViews" : [
      {
      ...
      {
        "buffer" : 0,
        "byteOffset" : padding + indexBufferLength,
...
function align(nodes, center, midwayBetweenEyes, noseTip, leftCheek, rightCheek) {

  //translate mesh such that middle of forehead = {0, 0, 0}
  const xOffset = 0-midwayBetweenEyes[0];
  const yOffset = 0-midwayBetweenEyes[1];
  const zOffset = 0-midwayBetweenEyes[2];
  const vector = [xOffset, yOffset, zOffset];
  nodes.forEach(node => {
    translateNode3D(node, vector);
  });
  console.log(midwayBetweenEyes);

  //rotate mesh about {0, 0, z} such that nose is {0, ?, ?}
  const roll = Math.atan(noseTip[0]/noseTip[1]);
  //rotate(nodes, 0, 0, roll);

  //rotate mesh about {x, 0, 0} such that nose is {0, ?, 0}
  const pitch = -Math.atan(noseTip[2]/noseTip[1]);
  //rotate(nodes, 0, pitch, 0);

  //rotate mesh about {0, y, 0} such that the leftCheek.z == rightCheek.z
  const diff = (leftCheek[2] - rightCheek[2])/2;
  const yaw = -Math.atan(diff/leftCheek[0]);
  rotate(nodes, 0, pitch, roll);

  nodes.forEach(node => {
    translateNode3D(node, center);
  });
}

function translateNode3D(node, vector) {
  node[0] += vector[0];
  node[1] += vector[1];
  node[2] += vector[2];
}
